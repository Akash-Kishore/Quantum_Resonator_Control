# phase3_evaluate_v5.py
import os, json, numpy as np
from stable_baselines3 import PPO
from stable_baselines3.common.vec_env import DummyVecEnv, VecNormalize
from qiskit_integration.qiskit_resonator_env_v3 import QiskitResonatorEnvV3

# Updated paths for V5
QFT_MODEL = (
    r"C:\MiniProject_Sem4\data_logs" r"\qiskit_aer_simulation_2\v4_qft_v5\best_model"
)
QFT_STATS = (
    r"C:\MiniProject_Sem4\data_logs"
    r"\qiskit_aer_simulation_2\v4_qft_v5\vec_normalize_qft_v5.pkl"
)
...
print(f"Evaluating V4_QFT_V5 over {N_EPISODES} episodes...")

results, trajectories = [], []
for ep in range(N_EPISODES):
    obs = env.reset()
    ep_amp, ep_err, ep_fp, ep_fq = [], [], [], []
    done = False
    while not done:
    ...
    if (ep + 1) % 100 == 0:
        cur_near = np.mean([r["near"] for r in results])
        print(
            f"  {ep+1}/{N_EPISODES} | "
            f'MAE {np.mean([r["mae"] for r in results]):.0f} Hz | '
            f"Near {cur_near*100:.1f}%"
        )
summary = {
    "phase": "v5_eval",
    "model": "V4_QFT_V5",
...
with open(os.path.join(OUT_DIR, "qft_v5_summary.json"), "w") as f:
    json.dump(summary, f, indent=2)
with open(os.path.join(OUT_DIR, "qft_v5_trajectories.json"), "w") as f:
    json.dump(trajectories, f)
print(f"\n=== V5 QFT RESULTS ===")
print(f"MAE:  {summary['mae_mean']:.0f} ± {summary['mae_std']:.0f} Hz")
print(f"Near: {summary['near_mean']*100:.1f}% ± {summary['near_std']*100:.1f}%")
print(f"Low:  {summary['low_mean']*100:.1f}%")
