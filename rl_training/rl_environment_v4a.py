...
class ResonatorEnv(gym.Env):
    """
    V4a ablation environment — 5-element observation space.
    Identical to V4 (rl_environment.py) except element[4] is raw delta_amp
    (ema_amp - prev_ema_amp) with NO normalisation by frequency step size.
    This isolates whether the normalisation in V4's amp_gradient is necessary.

    Observation: [norm_freq, ema_amp, prev_ema_amp, prev_action, raw_delta_amp]
    ...
    def __init__(self):
        ...
        # Element[4] is raw delta_amp: range [-1.2, 1.2] (amplitude difference, clipped)
        low  = np.array([0.0, 0.0, 0.0, -1.0, -1.2], dtype=np.float32)
        high = np.array([1.0, 1.2, 1.2,  1.0,  1.2], dtype=np.float32)
        self.observation_space = spaces.Box(low=low, high=high, dtype=np.float32)

        self.current_freq = 500e3
        self.raw_delta_amp = 0.0
        self.max_steps = 200
        self.current_step = 0
    ...
    def _get_obs(self):
        ...
        # V4a ablation: raw delta_amp, no normalisation by frequency step
        safe_raw_delta = np.clip(self.raw_delta_amp, -1.2, 1.2)

        assert len(np.array([norm_freq, safe_ema, safe_prev_ema,
                              safe_prev_action, safe_raw_delta])) == 5, \
            "V4a obs length must be 5"

        return np.array([norm_freq, safe_ema, safe_prev_ema,
                         safe_prev_action, safe_raw_delta], dtype=np.float32)
    ...
    def step(self, action):
        ...
        # V4a ablation: raw delta_amp only — no freq normalisation
        self.raw_delta_amp = self.ema_amp - self.prev_ema_amp
...
if __name__ == "__main__":
    env = ResonatorEnv()
    obs, _ = env.reset()
    print("V4a environment initialized. obs length:", len(obs))
